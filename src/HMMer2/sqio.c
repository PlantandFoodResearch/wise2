 ...
 */
static void 
getlineSeq(struct ReadSeqVars *V)
{
  char *cp;
...
readPIR(struct ReadSeqVars *V)
{
  ...
  getlineSeq(V);			/* skip next line, coords */
...
readPearson(struct ReadSeqVars *V)
{
  ...
  while (V->longline && ! feof(V->f)) getlineSeq(V);
...
readZuker(struct ReadSeqVars *V)
{
  ...
  getlineSeq(V);  /*s == "seqLen seqid string..."*/
...
readSquid(struct ReadSeqVars *V)
{
  ...
  while (strncmp(V->sbuffer, "NAM ", 4) != 0) getlineSeq(V);
